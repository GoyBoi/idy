...
export function WishlistIcon() {
  ...
  return (
    <Link to="/wishlist" className="p-2 rounded-full hover:bg-foreground/10 transition-colors relative" aria-label="Wishlist">
      ...
      {wishlist.length > 0 && (
        <span className="absolute -top-2 -right-2 bg-yellow-400 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center" style={{ border: '1px solid #dc7030' }}>
          {wishlist.length}
        </span>
      )}
    </Link>
